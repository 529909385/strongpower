...
<template>
  <div :style="{width:tableWidth}" class='autoTbale'>
    <table class="table table-bordered" id='hl-tree-table'>
      ...
      <tr>
        <th v-for="(column,index) in cloneColumns">
          <label v-if="column.type === 'selection'">
           ...
           <!-- <el-checkbox v-model="checks"></el-checkbox>-->
          </label>
          <label v-else>
      ...
      <tbody>
      <tr v-for="(item,index) in initItems" :key="item.id" v-show="show(item)" :class="{'child-tr':item.parent}" class="normal">
          ...
          <label @click="toggle(index,item)" v-if="!column.type">
                            <span v-if='snum==iconRow()'>
                                ...
                                <i v-if="item.children&&item.children.length>0" class="ivu-icon"
                                   :class="{'tree-open':!item.expanded,'tree-close':item.expanded }"></i>
                                <i v-else class="tree-file"></i>
...
/*  $(document).ready(function() {
    $('#hl-tree-table').on('click', '.normal', function() {
      $('#hl-tree-table .normal').removeClass('active')
  ...
  export default {
    ...
    methods: {
      ...
      tdStyle(column) {
        ...
        if (column.width) {
          if (column.type === 'selection' || column.type === 'action') {
...
<style scoped>
  ...
  input[type="checkbox"]:checked {
    ...
    border-color: #409EFF;
    background: url("../../../static/check/checked.png") no-repeat -1px -1px;
  }
  label {
    margin: 0 8px;
  }
  ...
  .tree-open{
    ...
    width: 15px;
    height: 15px;
    background: url("../../../static/treeIcons/tree_icons.png") no-repeat -208px 0;
  }
  .tree-close{
    cursor: pointer;
    display: inline-block;
    width: 15px;
    height: 15px;
    background: url("../../../static/treeIcons/tree_icons.png") no-repeat -224px 0;
  }
  .tree-file {
    display: inline-block;
    width: 16px;
    height: 16px;
    background: url("../../../static/treeIcons/tree_icons.png") no-repeat -240px 0;
  }
</style>
